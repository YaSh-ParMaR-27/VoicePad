...
class Notepad:
    ...
    def set_menu_bar(self):
        ...
        self.color_icons = (
            self.light_default_icon,
            self.light_plus_icon,
            self.dark_icon, self.red_icon,
            self.monokai_icon,
    ...
    def set_file_sub_menu(self):
        #compound attribute is used for separating text and image
    ...
    def set_edit_sub_menu(self):
        # unlike entry there is a 'Text' class which takes multiline input . This Text class has a method
        # called event_generate which generates the standard action on our text through the key passed as an argument.
        # Like control c for copy
    ...
    # ------------------------------*****************--------------------------------------#
    # These methods are called from __init__()
    ...
    # event = None is passed because we have done event handling of save as
    # 1) when user clicks on save button this is done through command attribute which does not pass event object
    # 2) we have also done event binding as when user clicks control + s
    #       then also file is saved but here an event object is passed to the method
    # So we have created a single method that runs on both the command and bind.

    # This method will be called when user clicks on the 'save' button in the file sub-menu menu or presses ctrl+s
    def save_file(self, event=None):
        try:
            # 1.0 is the 1st row and 0th column , it considers the first text as from 1st row
    ...
    # This method will be called when user clicks on the 'Save As' button in the file sub-menu menu or presses Alt+s
    ...
    # This method creates a new file i.e. it generates a new empty canvas
    # This method will be called when user clicks on the 'New' button in the file sub-menu menu or presses ctrl+N
    def new_file(self, event=None):
        self.file_controller.new_file()
        self.root.title("MyNotePad")
        self.text_editor.delete(1.0, tk.END)

    # This method is used to open a file / read file , If the file is secure user have to enter passord of the file
    # But if the file is not secure it will directly open

    # This method will be called when user clicks on the 'Open' button in the file sub-menu menu or presses ctrl+o
    ...
    # Whenever we will open secure files , It will ask user to enter the password of the file
    # This method is called internally from open_file
    ...
    # copy, paste , cut and clear all commands have been implemented in the set_edit_sub_menu() method.

    # Implementation of find
    # ----------------------

    # The find_func() to display the UI
    ...
    def find(self):
        ...
        # As the previously set tag will have foreground and background colors,
        # so when we search for new element, we will need to remove this tag
        # otherwise words with this tag will also have the same foreground and background colors
        # similar to the new searched element
        ...
        if word:
            ...
            while True:
                ...
                if not start_pos:
                    break
                end_pos = f'{start_pos}+{len(word)}c'  # c stands for column
    ...
    # changing font family i.e. font style
    ...
    def say_something(self):
        try:
            ...
            elif self.takeAudio.lower() == 'hey sonic open file':
                self.open_file()
            elif self.takeAudio.lower() == 'hey sonic save file':
                self.save_file()
            elif self.takeAudio.lower() == 'hey sonic new file':
                self.new_file()
            elif self.takeAudio.lower() == 'hey sonic file save':
                self.save_as()
            elif self.takeAudio.lower() == 'hey sonic close app':
                self.exit_func()
            elif self.takeAudio.lower() == 'hey sonic text bold':
                self.change_bold()
            elif self.takeAudio.lower() == 'hey sonic text italic':
                self.change_italic()
            elif self.takeAudio.lower() == 'hey sonic search':
                self.find_func()
            elif self.takeAudio.lower() == 'hey sonic hide statusbar':
                self.hide_statusbar()
            elif self.takeAudio.lower() == 'hey sonic hide toolbar':
                self.hide_toolbar()
...
# ==============================================================================================
    ...
    def add_file(self):
        try:
            if self.file_owner_input.get().strip() !="" and self.file_pwd_input.get().strip() != "":
                ...
                messagebox.showerror("Error!", "Please First Enter file owner name and password before adding the file")
        except:
            messagebox.showerror("DB Error","Error while adding file")
            print(traceback.format_exc())

    # opening the secure files
    def open_secure_file(self):
       try:
           ...
           messagebox.showerror('Secure File Error', "Error while opening the file")
           print(traceback.format_exc())

    # Removing the secure files
    ...
    # opening file on double click
    def list_double_click(self, e):
        self.open_secure_file()

    # ===============================================================================================
    # This method will run when we will write text on the canvas ,
    # The canvas has been bind with this method
